...
| C — le journal à la source | appliqué — le hook `journal_ha.py` écrit la ligne `appel` en base dès que `KOKAJI_BASE_URL` est là, et dans le fichier du jour sinon ou si la base ne répond pas (un appel ne se perd pas, et il le dit une fois) ; la passerelle devient l'image de LiteLLM plus le pilote (`dojo/litellm/Dockerfile`, `ensurepip`) ; `kokaji/corpus/journal.py` lit les deux sources ensemble sans compter un appel deux fois (l'identifiant d'appel fait foi) et la base par harness ; `JournalBase` (`appels`, `compter`) dans `base.py` ; verser, la veille et le QG demandent par harness, et le QG suit les sessions en cours sans exiger un dossier quand la base est là ; `KOKAJI_BASE_URL` passée à la passerelle, au service et aux deux veilles dans le compose du produit — vide par défaut, rien ne bascule ; tests : le hook sans base, avec base, avec base tombée, la lecture mêlée, et une veille qui capture un ha depuis un journal qui n'est qu'en base |
| D — promouvoir et exporter | appliqué — `kokaji promouvoir` retrouve le ha par le dépôt en service et, en régime base, écrit le `CAS-XXXX/` dans le clone avant de l'ajouter de force à git (D14.5) ; `kokaji instance exporter <dossier>` écrit tout ce que la base contient au format fichiers (`ha/<harness>/<corpus>/CAS-XXXX/`, écartés, `journal/` par jour, `manifeste.json` des chemins d'origine) ; `kokaji instance importer` prend un export ou le dossier des harness d'une instance en fichiers avec son `--journal` — la migration du §6 —, `--vers` transposant les chemins ; `kokaji/corpus/instance.py`, `DepotBase.corpus_connus` ; **la clé d'un corpus en base est son identité (`<harness>/<nom>`), pas son chemin de montage** (`enregistrer_corpus`/`enregistrer_cle`, correctif du lot F : deux conteneurs qui montent le même corpus à deux chemins écrivent le même corpus) ; `ecarter` idempotent en base ; sabotage 4 tenu par les tests : import d'une instance en fichiers puis export → chaque `CAS-XXXX/` et le journal identiques octet pour octet ; export puis import sur une base vide → mêmes ha, mêmes identités, et le QG en régime base, sans un fichier, rend sujet par sujet exactement ce qu'il rendait en fichiers. Les comptes ne sont pas dans l'export : ils sont encore un fichier SQLite, le lot E les y mettra |
| E — les comptes | appliqué — un seul magasin, deux moteurs (`kokaji/comptes/depot.py`) : le SQL écrit une fois avec des `?`, SQLite le joue tel quel, Postgres le traduit ; même schéma à trois lignes près, dites dans le module (l'unicité de l'email par un index sur `lower(email)`, un `rowid` déclaré pour l'ordre des contributeurs, la clé étrangère `harness_courant` non déclarée — aucune ligne de `harness_acl` n'est jamais effacée) ; `ou_ouvrir(chemin)` rend la base quand `KOKAJI_BASE_URL` est là, le service et les commandes passent par lui, le chemin `--comptes` reste ; `transvaser(de, vers)` recopie table par table, rejouable ; `kokaji instance importer --comptes <sqlite>` migre une fois, l'export rend `comptes.sqlite3` ; la suite des comptes (70 tests) se joue en CI une seconde fois sur Postgres, inchangée — c'est la mesure de « même schéma » |
| F — la première instance | appliqué (15 sept, prod `sha-71b624d`) — base `kokaji` dans le Postgres du dojo, `KOKAJI_BASE_URL` au compose privé, passerelle = image LiteLLM + pilote ; import de 169 ha (4 corpus), 1120 appels, comptes ; parité QG mesurée (22 sujets, 0 écart) avant bascule ; la bascule a révélé et fait corriger le défaut de clé de corpus (correctif ci-dessus) et l'a refaite propre : aucun corpus fantôme, écartés non doublés, clones propres, vigie 0 défaut. Entretien §7 en cours (fichiers gardés un mois) |

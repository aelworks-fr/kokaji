...
| D — promouvoir et exporter | appliqué — `kokaji promouvoir` retrouve le ha par le dépôt en service et, en régime base, écrit le `CAS-XXXX/` dans le clone avant de l'ajouter de force à git (D14.5) ; `kokaji instance exporter <dossier>` écrit tout ce que la base contient au format fichiers (`ha/<harness>/<corpus>/CAS-XXXX/`, écartés, `journal/` par jour, `manifeste.json` des chemins d'origine) ; `kokaji instance importer` prend un export ou le dossier des harness d'une instance en fichiers avec son `--journal` — la migration du §6 —, `--vers` transposant les chemins ; `kokaji/corpus/instance.py`, `DepotBase.corpus_connus` ; sabotage 4 tenu par les tests : import d'une instance en fichiers puis export → chaque `CAS-XXXX/` et le journal identiques octet pour octet ; export puis import sur une base vide → mêmes ha, mêmes identités, et le QG en régime base, sans un fichier, rend sujet par sujet exactement ce qu'il rendait en fichiers. Les comptes ne sont pas dans l'export : ils sont encore un fichier SQLite, le lot E les y mettra |

...
chaine:                       # topologie pour le QG (nœuds, jalons, arêtes)
  ...
  aretes: [ { de: "", vers: "", label: "", condition: "" } ]   # RFC-016 — condition déclarative sur le bloc d'état amont ; vide = passe toujours
...
- R2.8 — **Le routage est mécanique** ([RFC-016](docs/rfc-016-kata-d-action.md) D16.3). Une arête porte une `condition` déclarative sur le bloc d'état émis par le kata amont (`<chemin> <op> <valeur>` joints par `and`/`or` ; `verdict` et `nature` sont des commodités calculées, `resultat` un alias de la racine). Le dojo l'évalue — il ne juge pas. Une condition illisible est refusée au manifeste. À l'exécution : si aucune condition ne tranche, ou si plusieurs le font, la main revient à un **checkpoint humain** — jamais de branche devinée. L'exécution des actions et des vérificateurs se fait derrière une façade que l'instance câble sur son agent CLI ; par défaut elle est **inerte** (n'exécute rien, journalise l'intention), le temps qu'une RFC d'isolement tranche où un agent touche le monde.
